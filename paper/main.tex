...
\item An explicit statement of the preprocessing workflow as a staged procedure with defined inputs and outputs (Section~\ref{sec:workflow}), including a distribution-aware rule that assigns a scaler to each feature from training-fold statistics alone (Algorithm~\ref{alg:scaler}). In the earlier version this assignment was a hand-made table specific to one dataset; stating it as a rule makes it auditable and, more importantly, testable on other data --- which is how Section~\ref{sec:res-datasets} establishes that it does not transfer.
...
\item Three negative results, reported because they qualify the earlier version's claims and this one's: the feature-specific scaling strategy does not outperform uniform standardisation on this dataset; the distribution-aware scaler rule that replaced it does not transfer, holding the component ordering on only \NOrderHoldsAuto{} of \NDatasets{} datasets against \NOrderHoldsUni{} for plain standardisation, and carrying a negative Shapley value on Ames; and the derived features contribute little once selection and tuning are in place.
\end{enumerate}
...
At 3{,}000 rows the tuned SVR attains $R^2 = \SvrRbfSmall$ and ranks \SvrRankSmall{} of \NModels; on the full training set it attains $\SvrRbfFull$ and ranks \SvrRankFull{}. The three tree ensembles remain ahead of it in both settings: XGBoost ($\XgbFull$ on the full training set, $\XgbSmall$ at 3{,}000 rows), gradient boosting ($\GbmFull$, $\GbmSmall$) and random forest ($\RfFull$, $\RfSmall$). The equalisation therefore changes the SVR's absolute score substantially --- the 3{,}000-row restriction was costing it approximately $0.02$ in $R^2$ --- without changing the conclusion that gradient-boosted trees lead on this benchmark. This is the answer to the first half of Q2: a properly configured SVR is competitive but not leading, and the earlier version's ranking claim survives the correction, while the caveat that accompanied it in the Limitations section belongs next to the claim itself.
...
\input{tables/tab_ablation_shapley}
...
\input{tables/tab_ablation_cells}
...
Random cross-validation gives a mean $R^2$ of $\RandomCVmean$; spatially blocked cross-validation of the same pipeline gives $\SpatialCVmean \pm \SpatialCVsd$. The difference, $\SpatialOptimism$, is the amount by which a random split overstates performance on a region the model has not seen, and it is far larger than any of the pipeline effects measured in Section~\ref{sec:res-ablation}. Performing feature selection inside each fold rather than once on the whole training set changes the estimate by $\SelectionOptimism$ --- that is, not at all to four decimal places, under either partitioning scheme. The reason is that the ensemble ranking is completely stable here: every fold selects the same twelve features, so selecting inside the loop and selecting outside it are the same computation. This is a property of this dataset rather than a general reassurance, and it is reported because the earlier version selected features once, outside the loop, and had no way of knowing that it had got away with it. The fully nested estimates, in which both selection and hyperparameter search happen inside each outer fold, are $\NestedRandom$ under random partitioning and $\NestedSpatial$ under spatial partitioning. The second of these is not a small degradation but a negative $R^2$: averaged over five held-out geographic regions the nested model predicts worse than the training-set mean, because the five-block partition holds out regions large enough that the training folds contain no comparable market. The individual folds range from $\NestedSpatialMax$ down to $\NestedSpatialMin$ depending on which region is withheld, against $\NestedRandomMin$ to $\NestedRandomMax$ under random partitioning. Figure~\ref{fig:validation} shows the spatial blocks and the resulting estimates.
...
The answer depends on which scaling strategy the workflow uses, and this is the third negative result of the study.

Under plain uniform standardisation the ordering is reproduced on all \NOrderHoldsUni{} of \NDatasets{} datasets. Scaling is the largest component on California Housing ($\DSCaliforniaUniScal$), King County ($\DSKcHouseUniScal$) and Ames ($\DSAmesUniScal$); tuning is second ($\DSCaliforniaUniTune$, $\DSKcHouseUniTune$, $\DSAmesUniTune$) and selection last ($\DSCaliforniaUniSelect$, $\DSKcHouseUniSelect$, $\DSAmesUniSelect$). To the extent that this paper supports any workflow-level statement, this is it: with a conventional scaler, the ordering scaling $>$ tuning $>$ selection holds on three housing datasets of very different size and width.

Under the distribution-aware scaler rule of Algorithm~\ref{alg:scaler} it holds on only \NOrderHoldsAuto{} of \NDatasets{}. On King County the rule makes scaling the \emph{smallest} contributor ($\DSKcHouseScal$ against $\DSKcHouseTune$ for tuning), and on Ames its Shapley value is \emph{negative} ($\DSAmesScal$): applying the rule first, before selection or tuning, costs $0.6455$ in $R^2$ on that dataset. Ames is the smallest and widest of the three ($\DSAmesN$ training rows, $\DSAmesP$ candidate features), which is precisely the regime in which per-feature scaler assignment estimated from training statistics is least reliable --- each decision is made from less data, and a single misassigned heavy-tailed feature propagates through the kernel.

The consequence for this paper is direct. Section~\ref{sec:res-reproduction} already showed that the hand-assigned feature-specific strategy does not beat uniform standardisation on California Housing; the present result shows that its rule-based successor does not transfer either. The scaling stage of the workflow should therefore be read as ``apply a conventional scaler'', and Algorithm~\ref{alg:scaler} is reported here as a component that was tested and did not earn its place, rather than as a contribution. Figure~\ref{fig:datasets} shows both attributions side by side.

\begin{figure}[htbp]
\centering
\includegraphics[width=0.8\textwidth]{figures/fig_datasets.pdf}
\caption{Shapley attribution of the three workflow components on each dataset, under uniform standardisation (left) and under the distribution-aware scaler rule of Algorithm~\ref{alg:scaler} (right). Under uniform standardisation the ordering scaling $>$ tuning $>$ selection is preserved on all three datasets; under the rule it is not, and on Ames the scaling contribution is negative.}
\label{fig:datasets}
\end{figure}
...
Two of the three constants are not load-bearing and one is, and the distinction matters.

The ensemble weights are not. Across the \WeightN{} weight combinations on the simplex, test $R^2$ varies between \WeightMin{} and \WeightMax{}, a range of \WeightRange{}, and the smallest overlap between any swept selection and the default top-twelve set is \WeightMinOverlap{} of twelve features: the ranking is largely insensitive to how its three signals are weighted, so the 40/30/30 split is a presentational choice rather than a tuned one. The thresholds of Algorithm~\ref{alg:scaler} are not either --- sweeping them over nine combinations moves $R^2$ by \ThreshRange{}, which is negligible.

The number of selected features \emph{is}. Varying $k$ from \KminAt{} to \KmaxAt{} moves $R^2$ between \KrangeLo{} and \KrangeHi{}, a spread of almost $0.2$, which is larger than any component effect reported in Section~\ref{sec:res-ablation}. The dependence is not smooth: $R^2$ climbs steeply up to $k = \KplateauFrom$ and is flat thereafter, varying by only \KplateauRange{} across every $k$ from \KplateauFrom{} to \KmaxAt{}. At $k = \KminAt$ the model reaches only \KbelowPlateau. The defensible statement is therefore not that $k$ does not matter, but that it matters until roughly ten features are retained and not afterwards; the default of twelve sits on the plateau, and any choice on that plateau would serve equally well. A study that had chosen $k$ below the plateau would have drawn different conclusions about every other component, which is the reason this sweep is reported rather than the constants simply being asserted. Figure~\ref{fig:sensitivity} shows both sweeps.
...
Table~\ref{tab:kernels} tunes each kernel over its own hyperparameters rather than tuning only the RBF kernel, which was a limitation of the earlier version. Table~\ref{tab:search} compares the exhaustive grid with randomised search at three budgets. The \GridFits{}-fit grid reaches $R^2 = \GridRtwo$ in \GridSecs{}~s (\GridSecsMin{}~minutes); randomised search reaches \RandTwentyRtwo{} in \RandTwentySecs{}~s at 20 iterations, \RandSixtyRtwo{} in \RandSixtySecs{}~s at 60, and \RandTwoHundredRtwo{} in \RandTwoHundredSecs{}~s at 200. Two things follow. First, the exhaustive grid is entirely affordable on this problem --- minutes, not hours --- so Reviewer~2 was right that there was no computational case for avoiding it, and it is reported here alongside the randomised search rather than argued away. Second, the returns are not monotone in the budget: the best result comes from \SearchBest{} iterations, and raising the budget to 200 costs \RandTwoHundredSecs{}~s to arrive at a slightly \emph{worse} score than 60 iterations reached in \RandSixtySecs{}~s. That non-monotonicity is single-split sampling noise rather than a property of the search, which is itself the reason the significance testing of Section~\ref{sec:res-stats} is done over repeated splits rather than on one. The revision uses the larger budget throughout, not because more iterations reliably help, but because the cost of removing the question is a few minutes.

\input{tables/tab_kernels}
\input{tables/tab_search}
...
Within the California Housing dataset and the configurations tested here, the evidence supports three statements. First, the poor SVR result reported by Preethi et al.\ is reproducible from the configuration they describe, and is removed by adding a scaling step to that same configuration: the difference between an unscaled and a standardised pipeline, holding everything else constant, is roughly $0.75$ in $R^2$. Second, once every model receives equal data and an equal tuning budget, a properly configured SVR is a mid-ranked model on this benchmark --- clearly ahead of the linear family and the single decision tree, clearly behind gradient-boosted trees --- and the differences against the leading models survive a corrected paired test. Third, under uniform standardisation the components of the workflow contribute in the order scaling, tuning, selection, and this ordering is reproduced on two further housing datasets --- though, as Section~\ref{sec:res-datasets} shows, it is not reproduced when the distribution-aware scaler rule is used in place of a conventional one.

\subsection{What the evidence does not support}\label{sec:disc-not}
Three claims made in the earlier version are not supported and are withdrawn. The feature-specific scaling strategy is not better than plain standardisation on this dataset; it is measurably worse, and the workflow's scaling stage is best described as ``any reasonable scaling'' rather than as a specific advance. A fourth claim, made in the present revision rather than the earlier version, is also withdrawn: the distribution-aware scaler rule of Algorithm~\ref{alg:scaler} was introduced here as the auditable, transferable successor to that hand-made table, and Section~\ref{sec:res-datasets} shows it is not transferable. It preserves the component ordering on \NOrderHoldsAuto{} of \NDatasets{} datasets where plain standardisation preserves it on \NOrderHoldsUni{}, and on Ames its contribution is negative. A rule that must be checked against a conventional scaler on every new dataset before it can be trusted offers nothing over the conventional scaler, and it is reported here as a tested and rejected component. The attribution of 95.7\% of the improvement to scaling is an artefact of a single ablation ordering; the order-independent figure is substantially lower. And the claim that the cross-validation interval excludes a competing value rested on an incorrect interval formula.
...
This paper re-examined a published finding that Support Vector Regression is the weakest of six regression models on the California Housing benchmark. Reading the source correctly, the reported SVR result is $R^2 \approx \PreethiReportedRtwo$ rather than the $0.60$ quoted in an earlier version of this work, and the configuration described there reproduces at $\RepUnscaledDefault$ to $\RepUnscaledTuned$ depending on the search budget. Adding a standardisation step to that same pipeline raises it to $\RepScaledTuned$. A four-factor ablation evaluated in all \AblNcells{} cells attributes $\ShapScal$ of the improvement to scaling, $\ShapTune$ to tuning, $\ShapSelect$ to selection and $\ShapDeriv$ to the derived features, in a way that does not depend on the order in which the components are applied; under uniform standardisation the same ordering is reproduced on King County and Ames housing data, but under the distribution-aware scaler rule proposed here it is not, and that rule is reported as a component that failed to transfer. With equal training data and an equal tuning budget the tuned SVR ranks \SvrRankFull{} of \NModels{} models, behind gradient-boosted trees, and the paired differences are significant under the Nadeau--Bengio correction. All of these figures are obtained under random partitioning. Cross-validating the same pipeline over spatially contiguous blocks instead lowers its estimate from $\RandomCVmean$ to $\SpatialCVmean$, a gap of $\SpatialOptimism$ that is larger than any pipeline effect measured in this study, and the fully nested spatial estimate is negative at $\NestedSpatial$. The numbers reported here, like those of the literature they revisit, describe interpolation within a sampled region.
