...
usable numerical predictors). Shapley attributions for scaling are $\DSCaliforniaScal$,
$\DSKcHouseScal$ and $\DSAmesScal$; for tuning $\DSCaliforniaTune$, $\DSKcHouseTune$ and
$\DSAmesTune$; for selection $\DSCaliforniaSelect$, $\DSKcHouseSelect$ and $\DSAmesSelect$.
The \emph{ordering} of the components --- scaling, then tuning, then selection --- is
reproduced on all three datasets; the magnitudes are not, and I say so. I also state the
limit of this evidence in Section~\ref{sec:disc-threats}: all three are residential-property datasets, so this
tests transfer across datasets within a domain, not across domains.
...
twelve; varying $k$ from 4 to 18 moves $R^2$ between \KrangeLo{} and \KrangeHi{}; varying the
scaler thresholds over nine combinations moves it by \ThreshRange{}. The honest conclusion is
that these constants are not load-bearing, which I take to be the appropriate justification
for constants with no theoretical derivation.
...
ordering of the components is reproduced on both while the magnitudes are not. Independently
...
iterations and $\RandTwoHundredRtwo$ at 200. Since the search costs seconds, the larger
budget is used throughout the revision and the small-budget limitation is removed rather than
defended. Every kernel is now tuned over its own hyperparameters, not only the RBF
